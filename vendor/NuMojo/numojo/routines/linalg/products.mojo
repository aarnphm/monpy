...
def dot[
    ...
    if array1.ndim == array2.ndim == 1:
        ...
        vectorize[width, vectorized_dot](array1.size)
        return result^
    else:
        raise Error(
            "resultross product is not supported for arrays of shape "
    ...
    def calculate_A_rows(m: Int):
        ...
        def calc_tile[tile_x: Int, tile_y: Int](x: Int, y: Int):
            for k in range(y, y + tile_y):
                ...
                vectorize[width, dot, unroll_factor=unroll_factor](tile_x)
    ...
    if A.is_c_contiguous() and B.is_c_contiguous():
        ...
        def calculate_FF(n: Int):
            for k in range(A.shape[1]):
                ...
                vectorize[width, dot_F](A.shape[0])
        ...
        def calculate_resultF(m: Int):
            for n in range(B.shape[1]):
                ...
                def dot_product[
                    simd_width: Int
                ](k: Int) capturing -> None:
                    sum += (
                        A._load[simd_width](m, k) * B._load[simd_width](k, n)
                    ).reduce_add()

                vectorize[width, dot_product](A.shape[1])

...
struct Matrix[
    ...
    def __pow__(self, rhs: Scalar[Self.dtype]) raises -> Matrix[Self.dtype]:
        ...
        vectorize[width, vec_pow](self.size)
        return result^
    ...
    def __iadd__(mut self, other: Matrix[Self.dtype]) raises:
        ...
        if self.is_c_contiguous() and other.is_c_contiguous():
            ...
            @parameter
            def vec_add[w: Int](i: Int) capturing:
            ...
            vectorize[width, vec_add](self.size)
        else:
            for i in range(self.shape[0]):
                for j in range(self.shape[1]):
    ...
    def __iadd__(mut self, other: Scalar[Self.dtype]):
        ...
        if self.is_c_contiguous():
            ...
            @parameter
            def vec_add_scalar[w: Int](i: Int) capturing:
                var a = self._buf.ptr.load[width=w](self.offset + i)
                self._buf.ptr.store(self.offset + i, a + other)

            vectorize[width, vec_add_scalar](self.size)
        else:
            for i in range(self.shape[0]):
                for j in range(self.shape[1]):
    ...
    def __isub__(mut self, other: Matrix[Self.dtype]) raises:
        ...
        if self.is_c_contiguous() and other.is_c_contiguous():
            ...
            @parameter
            def vec_sub[w: Int](i: Int) capturing:
                var a = self._buf.ptr.load[width=w](self.offset + i)
                var b = other._buf.ptr.load[width=w](other.offset + i)
                self._buf.ptr.store(self.offset + i, a - b)

            vectorize[width, vec_sub](self.size)
        else:
            for i in range(self.shape[0]):
                for j in range(self.shape[1]):
                    self._store(i, j, self._load(i, j) - other._load(i, j))
    ...
    def __isub__(mut self, other: Scalar[Self.dtype]):
        ...
        if self.is_c_contiguous():
            ...
            @parameter
            def vec_sub_scalar[w: Int](i: Int) capturing:
                var a = self._buf.ptr.load[width=w](self.offset + i)
                self._buf.ptr.store(self.offset + i, a - other)

            vectorize[width, vec_sub_scalar](self.size)
        else:
            for i in range(self.shape[0]):
                for j in range(self.shape[1]):
                    self._store(i, j, self._load(i, j) - other)
    ...
    def __imul__(mut self, other: Matrix[Self.dtype]) raises:
        ...
        if self.is_c_contiguous() and other.is_c_contiguous():
            ...
            @parameter
            def vec_mul[w: Int](i: Int) capturing:
                var a = self._buf.ptr.load[width=w](self.offset + i)
                var b = other._buf.ptr.load[width=w](other.offset + i)
                self._buf.ptr.store(self.offset + i, a * b)

            vectorize[width, vec_mul](self.size)
        else:
            for i in range(self.shape[0]):
                for j in range(self.shape[1]):
                    self._store(i, j, self._load(i, j) * other._load(i, j))
    ...
    def __imul__(mut self, other: Scalar[Self.dtype]):
        ...
        if self.is_c_contiguous():
            ...
            @parameter
            def vec_mul_scalar[w: Int](i: Int) capturing:
                var a = self._buf.ptr.load[width=w](self.offset + i)
                self._buf.ptr.store(self.offset + i, a * other)

            vectorize[width, vec_mul_scalar](self.size)
        else:
            for i in range(self.shape[0]):
                for j in range(self.shape[1]):
                    self._store(i, j, self._load(i, j) * other)
    ...
    def __itruediv__(mut self, other: Matrix[Self.dtype]) raises:
        ...
        if self.is_c_contiguous() and other.is_c_contiguous():
            ...
            @parameter
            def vec_div[w: Int](i: Int) capturing:
                var a = self._buf.ptr.load[width=w](self.offset + i)
                var b = other._buf.ptr.load[width=w](other.offset + i)
                self._buf.ptr.store(self.offset + i, a / b)

            vectorize[width, vec_div](self.size)
        else:
            for i in range(self.shape[0]):
                for j in range(self.shape[1]):
                    self._store(i, j, self._load(i, j) / other._load(i, j))
    ...
    def __itruediv__(mut self, other: Scalar[Self.dtype]):
        ...
        if self.is_c_contiguous():
            ...
            @parameter
            def vec_div_scalar[w: Int](i: Int) capturing:
                var a = self._buf.ptr.load[width=w](self.offset + i)
                self._buf.ptr.store(self.offset + i, a / other)

            vectorize[width, vec_div_scalar](self.size)
        else:
            for i in range(self.shape[0]):
                for j in range(self.shape[1]):
                    self._store(i, j, self._load(i, j) / other)
    ...
    def __ifloordiv__(mut self, other: Matrix[Self.dtype]) raises:
        ...
        if self.is_c_contiguous() and other.is_c_contiguous():
            ...
            @parameter
            def vec_floordiv[w: Int](i: Int) capturing:
                var a = self._buf.ptr.load[width=w](self.offset + i)
                var b = other._buf.ptr.load[width=w](other.offset + i)
                self._buf.ptr.store(self.offset + i, a // b)

            vectorize[width, vec_floordiv](self.size)
        else:
            for i in range(self.shape[0]):
                for j in range(self.shape[1]):
                    self._store(i, j, self._load(i, j) // other._load(i, j))
    ...
    def __ifloordiv__(mut self, other: Scalar[Self.dtype]):
        ...
        if self.is_c_contiguous():
            ...
            vectorize[width, vec_floordiv_scalar](self.size)
        else:
            for i in range(self.shape[0]):
                for j in range(self.shape[1]):
                    self._store(i, j, self._load(i, j) // other)
    ...
    def __imod__(mut self, other: Matrix[Self.dtype]) raises:
        ...
        if self.is_c_contiguous() and other.is_c_contiguous():
            ...
            @parameter
            def vec_mod[w: Int](i: Int) capturing:
                var a = self._buf.ptr.load[width=w](self.offset + i)
                var b = other._buf.ptr.load[width=w](other.offset + i)
                self._buf.ptr.store(self.offset + i, a % b)

            vectorize[width, vec_mod](self.size)
        else:
            for i in range(self.shape[0]):
                for j in range(self.shape[1]):
                    self._store(i, j, self._load(i, j) % other._load(i, j))
    ...
    def __imod__(mut self, other: Scalar[Self.dtype]):
        ...
        if self.is_c_contiguous():
            ...
            @parameter
            def vec_mod_scalar[w: Int](i: Int) capturing:
                var a = self._buf.ptr.load[width=w](self.offset + i)
                self._buf.ptr.store(self.offset + i, a % other)

            vectorize[width, vec_mod_scalar](self.size)
        else:
            for i in range(self.shape[0]):
                for j in range(self.shape[1]):
                    self._store(i, j, self._load(i, j) % other)
    ...
    def __lt__(self, other: Matrix[Self.dtype]) raises -> Matrix[DType.bool]:
        ...
        if (self.shape[0] == other.shape[0]) and (
            ...
            return _logic_func_matrix_matrix_to_matrix[Self.dtype, simd_lt](
                self, other
            )
        elif (self.shape[0] < other.shape[0]) or (
            self.shape[1] < other.shape[1]
    ...
    def __le__(self, other: Matrix[Self.dtype]) raises -> Matrix[DType.bool]:
        ...
        if (self.shape[0] == other.shape[0]) and (
            ...
            return _logic_func_matrix_matrix_to_matrix[Self.dtype, simd_le](
                self, other
            )
        elif (self.shape[0] < other.shape[0]) or (
            self.shape[1] < other.shape[1]
    ...
    def __gt__(self, other: Matrix[Self.dtype]) raises -> Matrix[DType.bool]:
        ...
        if (self.shape[0] == other.shape[0]) and (
            ...
            return _logic_func_matrix_matrix_to_matrix[Self.dtype, simd_gt](
                self, other
            )
        elif (self.shape[0] < other.shape[0]) or (
            self.shape[1] < other.shape[1]
    ...
    def __ge__(self, other: Matrix[Self.dtype]) raises -> Matrix[DType.bool]:
        ...
        if (self.shape[0] == other.shape[0]) and (
            ...
            return _logic_func_matrix_matrix_to_matrix[Self.dtype, simd_ge](
                self, other
            )
        elif (self.shape[0] < other.shape[0]) or (
            self.shape[1] < other.shape[1]
    ...
    def __eq__(self, other: Matrix[Self.dtype]) raises -> Matrix[DType.bool]:
        ...
        if (self.shape[0] == other.shape[0]) and (
            ...
            return _logic_func_matrix_matrix_to_matrix[Self.dtype, simd_eq](
                self, other
            )
        elif (self.shape[0] < other.shape[0]) or (
            self.shape[1] < other.shape[1]
    ...
    def __ne__(self, other: Matrix[Self.dtype]) raises -> Matrix[DType.bool]:
        ...
        if (self.shape[0] == other.shape[0]) and (
            ...
            return _logic_func_matrix_matrix_to_matrix[Self.dtype, simd_ne](
                self, other
            )
        elif (self.shape[0] < other.shape[0]) or (
            self.shape[1] < other.shape[1]
    ...
    def cumprod(self, axis: Int) raises -> Matrix[Self.dtype]:
        ...
        Example:
            ...
            ```
        """
        return products.cumprod(self, axis=axis)
    ...
    def cumsum(self, axis: Int) raises -> Matrix[Self.dtype]:
        ...
        Example:
            ...
            ```
        """
        return sums.cumsum(self, axis=axis)
    ...
    def std[
        ...
        Example:
            ...
            ```
        """
        return statistics.stddev[returned_dtype](self, ddof=ddof)
    ...
    def std[
        ...
        Example:
            ...
            ```
        """
        return statistics.stddev[returned_dtype](self, axis=axis, ddof=ddof)
    ...
    def sum(self, axis: Int) raises -> Matrix[Self.dtype]:
        ...
        Example:
            ...
            ```
        """
        return sums.sum(self, axis=axis)
    ...
    def full[
        ...
        @parameter
        def vec_fill[w: Int](i: Int) capturing:
            matrix._buf.ptr.store(i, SIMD[datatype, w](fill_value))

        vectorize[width, vec_fill](matrix.size)
        return matrix^
...
def _arithmetic_func_matrix_matrix_to_matrix[
    dtype: DType,
    simd_func: def[kernel_dtype: DType, simd_width: Int](
        SIMD[kernel_dtype, simd_width], SIMD[kernel_dtype, simd_width]
    ) capturing -> SIMD[kernel_dtype, simd_width],
    ...
    @parameter
    def vec_func[simd_width: Int](i: Int) capturing:
        res._buf.ptr.store(
            i,
            simd_func[dtype, simd_width](
                A._buf.ptr.load[width=simd_width](i),
                B._buf.ptr.load[width=simd_width](i),
            ),
        )

    vectorize[simd_width, vec_func](A.size)
    return res^
...
def _arithmetic_func_matrix_to_matrix[
    ...
    def vec_func[simd_width: Int](i: Int) capturing:
        C._buf.ptr.store(
            i, simd_func[dtype, simd_width](A._buf.ptr.load[width=simd_width](i))
        )
...
def _logic_func_matrix_matrix_to_matrix[
    ...
    @parameter
    def vec_compare[w: Int](i: Int) capturing:
    ...
    vectorize[width, vec_compare](A.size)
    return C^

...
- `src/create/` owns every python-callable op dispatcher. the current split is `ops/creation.mojo`, `ops/dtype.mojo`, `ops/elementwise.mojo`, `ops/linalg.mojo`, `ops/nn.mojo`, `ops/reduction.mojo`, and `ops/shape.mojo`. a top-level `linalg.mojo` would shadow Mojo's stdlib `linalg`, so the linalg dispatchers live here.

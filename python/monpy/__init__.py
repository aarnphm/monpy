...
DTYPE_BOOL, DTYPE_INT64, DTYPE_FLOAT32, DTYPE_FLOAT64 = (
  _DTCODES["BOOL"],
  _DTCODES["INT64"],
  _DTCODES["FLOAT32"],
  _DTCODES["FLOAT64"],
...
DTYPE_UINT64, DTYPE_UINT32, DTYPE_UINT16, DTYPE_UINT8 = (
  _DTCODES["UINT64"],
  _DTCODES["UINT32"],
  _DTCODES["UINT16"],
  _DTCODES["UINT8"],
...
(
  DTYPE_KIND_BOOL,
  DTYPE_KIND_SIGNED_INT,
  DTYPE_KIND_REAL_FLOAT,
  DTYPE_KIND_UNSIGNED_INT,
  DTYPE_KIND_COMPLEX_FLOAT,
  DTYPE_KIND_QUANT_FLOAT,
) = (
  _DTKCODES["BOOL"],
  _DTKCODES["SIGNED_INT"],
  _DTKCODES["REAL_FLOAT"],
  _DTKCODES["UNSIGNED_INT"],
  _DTKCODES["COMPLEX_FLOAT"],
  _DTKCODES["QUANT_FLOAT"],
)
CASTING_NO, CASTING_EQUIV, CASTING_SAFE, CASTING_SAME_KIND, CASTING_UNSAFE = (
  _CASTCODES["NO"],
  _CASTCODES["EQUIV"],
  _CASTCODES["SAFE"],
  _CASTCODES["SAME_KIND"],
  _CASTCODES["UNSAFE"],
)
OP_ADD, OP_SUB, OP_MUL, OP_DIV = _BINCODES["ADD"], _BINCODES["SUB"], _BINCODES["MUL"], _BINCODES["DIV"]
OP_FLOOR_DIV, OP_MOD, OP_POWER, OP_MAXIMUM, OP_MINIMUM, OP_FMIN, OP_FMAX, OP_ARCTAN2, OP_HYPOT, OP_COPYSIGN = (
  _BINCODES["FLOOR_DIV"],
  _BINCODES["MOD"],
  _BINCODES["POWER"],
  _BINCODES["MAXIMUM"],
...
PRED_ISNAN, PRED_ISINF, PRED_ISFINITE, PRED_SIGNBIT = (
  _PREDCODES["ISNAN"],
  _PREDCODES["ISINF"],
  _PREDCODES["ISFINITE"],
  _PREDCODES["SIGNBIT"],
)
REDUCE_SUM, REDUCE_MEAN, REDUCE_MIN, REDUCE_MAX, REDUCE_ARGMAX, REDUCE_PROD, REDUCE_ALL, REDUCE_ANY, REDUCE_ARGMIN = (
  _REDCODES["SUM"],
  _REDCODES["MEAN"],
  _REDCODES["MIN"],
  _REDCODES["MAX"],
...
class DType:
  ...
  def __eq__(self, other: object) -> builtins.bool:
    ...
    try:
      return self is _resolve_dtype(other)
  ...
  @property
  def type(self) -> builtins.type[object]:
...
_DT: tuple[DType, ...] = (
  ...
  float8_e8m0fnu,
  float4_e2m1fn,
)  # ordered by .code; idx == code
_DTC: dict[int, DType] = {d.code: d for d in _DT}  # code → DType
_DTN: dict[str, DType] = {d.name: d for d in _DT} | {
  "bool_": bool,
  "int_": int64,
  "float_": float64,
  "single": float32,
...
_DTK: dict[DType, int] = {
  ...
  int32: DTYPE_KIND_SIGNED_INT,
  int16: DTYPE_KIND_SIGNED_INT,
  int8: DTYPE_KIND_SIGNED_INT,
  uint64: DTYPE_KIND_UNSIGNED_INT,
  uint32: DTYPE_KIND_UNSIGNED_INT,
  uint16: DTYPE_KIND_UNSIGNED_INT,
  uint8: DTYPE_KIND_UNSIGNED_INT,
  float32: DTYPE_KIND_REAL_FLOAT,
  float64: DTYPE_KIND_REAL_FLOAT,
  float16: DTYPE_KIND_REAL_FLOAT,
  bfloat16: DTYPE_KIND_REAL_FLOAT,
  complex64: DTYPE_KIND_COMPLEX_FLOAT,
  complex128: DTYPE_KIND_COMPLEX_FLOAT,
  float8_e4m3fn: DTYPE_KIND_QUANT_FLOAT,
  float8_e4m3fnuz: DTYPE_KIND_QUANT_FLOAT,
  float8_e5m2: DTYPE_KIND_QUANT_FLOAT,
...
_CASTING_CODES: dict[str, int] = {
  ...
  "equiv": CASTING_EQUIV,
  "safe": CASTING_SAFE,
  "same_kind": CASTING_SAME_KIND,
  "unsafe": CASTING_UNSAFE,
}
_ISDTYPE_KINDS: dict[str, set[DType]] = {
  "bool": {bool},
  "signed integer": {int64, int32, int16, int8},
  "unsigned integer": {uint64, uint32, uint16, uint8},
  "integral": {int64, int32, int16, int8, uint64, uint32, uint16, uint8},
  "real floating": {float32, float64, float16, bfloat16},
  "quantized floating": {float8_e4m3fn, float8_e4m3fnuz, float8_e5m2, float8_e5m2fnuz, float8_e8m0fnu, float4_e2m1fn},
...
_FINFO: dict[DType, _FInfo] = {
  ...
  float8_e4m3fn: _FInfo(
    float8_e4m3fn, 8, 0.125, 0.0625, 448.0, -448.0, 0.015625, 0.015625, 1e-01, 1, 3, 4, 8, -6, -3, -4
  ),
  float8_e4m3fnuz: _FInfo(
    float8_e4m3fnuz, 8, 0.125, 0.0625, 240.0, -240.0, 0.0078125, 0.0078125, 1e-01, 1, 3, 4, 8, -7, -3, -4
  ),
  float8_e5m2: _FInfo(
    float8_e5m2, 8, 0.25, 0.125, 57344.0, -57344.0, 6.103515625e-05, 6.103515625e-05, 1e-01, 0, 2, 5, 16, -14, -2, -3
  ),
  float8_e5m2fnuz: _FInfo(
    float8_e5m2fnuz,
    8,
    0.25,
    0.125,
...
_IINFO: dict[DType, _IInfo] = {
  int64: _IInfo(int64, 64, -9223372036854775808, 9223372036854775807),
  int32: _IInfo(int32, 32, -2147483648, 2147483647),
  int16: _IInfo(int16, 16, -32768, 32767),
  int8: _IInfo(int8, 8, -128, 127),
  uint64: _IInfo(uint64, 64, 0, 18446744073709551615),
  uint32: _IInfo(uint32, 32, 0, 4294967295),
  uint16: _IInfo(uint16, 16, 0, 65535),
  uint8: _IInfo(uint8, 8, 0, 255),
}
...
_TBL = lambda s: {
  (_DT[i // 4], _DT[i % 4]): _DT[int(c)] for i, c in enumerate(s)
}  # decode digit-string → {(lhs,rhs): result}
...
_BR: dict[int, dict[tuple[DType, DType], DType]] = {
  OP_ADD: _NPT,
  OP_SUB: _NPT,
  OP_MUL: _NPT,
  OP_DIV: _DPT,
...
_UR: dict[DType, DType] = {
  ...
  float8_e8m0fnu: float32,
  float4_e2m1fn: float32,
  complex64: complex64,
  complex128: complex128,
}  # unary (sin/cos/exp/log) result: int kinds → f64
_UR_PRESERVE: dict[DType, DType] = {
  ...
  complex64: complex64,
  complex128: complex128,
}  # preserve-kind unary: bool → int64
...
def _iter(v: object) -> Iterable[typing.Any]:
  return typing.cast(Iterable[typing.Any], v)


def _py_int(v: object) -> int:
  return builtins.int(typing.cast(typing.Any, v))


def _py_float(v: object) -> float:
  return builtins.float(typing.cast(typing.Any, v))
...
def _axis_int(axis: object, ndim: int, context: str) -> int:
  if not isinstance(axis, builtins.int):
    raise TypeError(f"{context}: axis must be int")
  ax = axis + ndim if axis < 0 else axis
  if ax < 0 or ax >= ndim:
    raise ValueError(f"{context}: axis out of range")
  return ax
...
class Ufunc:
  ...
  def __call__(
    ...
    *args: object,
    ...
    where: object = True,
    casting: typing.Literal["no", "equiv", "safe", "same_kind", "unsafe"] = "same_kind",
    dtype: object = None,
  ) -> object:
    if (
      self.nin == 2
      ...
      and dtype is None
      and where is True
      and casting == "same_kind"
      and len(args) == 2
      and type(args[0]) is ndarray
      and type(args[1]) is ndarray
      and type(out) is ndarray
      and self._kind == "binary"
    ):
      _native.binary_into(out._native, args[0]._native, args[1]._native, self._op)
      return out
    if _has_kernel_arg(args):
      if out is not None or where is not True or dtype is not None:
        raise NotImplementedError(f"{self.__name__}: staged ufunc out/where/dtype are not implemented")
      from ._src.lax import primitives as _kernel_dsl

      return typing.cast(ndarray, _kernel_dsl.ufunc(self.__name__, *args))
    if where is not True:
      raise NotImplementedError(f"{self.__name__}: where= not implemented")
    if casting not in ("no", "equiv", "safe", "same_kind", "unsafe"):
      raise ValueError(f"unknown casting: {casting}")
    if len(args) != self.nin:
      raise TypeError(f"{self.__name__}() expected {self.nin} positional args, got {len(args)}")
    if self.nin == 1:
      return self._call_unary(args[0], out=out, dtype=dtype)
    if self.nin == 2:
      return self._call_binary(args[0], args[1], out=out, dtype=dtype)
  ...
  def _call_unary(self, x: object, *, out: ndarray | None, dtype: object) -> object:
    ...
    if (
      self._kind == "unary"
      and self._op in (UNARY_SIN, UNARY_COS, UNARY_EXP, UNARY_LOG)
      and out is None
      and dtype is None
    ):
      if isinstance(av, _DeferredArray):
        return typing.cast(ndarray, _UnaryExpression(av, self._op))
    ...
    if dtype is not None:
      ...
      if a.dtype != t:
        a = a.astype(t)
    if self._kind == "predicate":
      res = ndarray(_native.predicate(a._native, self._op))
      ...
      res = ndarray(_native.unary(a._native, self._op))
    if out is not None:
      out[...] = res
      return out
  ...
  def _call_binary(self, x1: object, x2: object, *, out: ndarray | None, dtype: object) -> object:
    ...
    if self._kind == "logical":
      ...
      try:
        t = _BR[OP_ADD][(l.dtype, r.dtype)]
      except KeyError:
        t = l.dtype if l.dtype == r.dtype else result_type(l, r)
      if l.dtype != t:
        l = l.astype(t)
      if r.dtype != t:
        r = r.astype(t)
  ...
  def accumulate(self, a: object, axis: int = 0, dtype: object = None, out: ndarray | None = None) -> ndarray:
    ...
    n = arr.ndim
    if n == 0:
      raise TypeError(f"{self.__name__}.accumulate: input is 0-d")
    ax = axis + n if axis < 0 else axis
    if ax < 0 or ax >= n:
      raise ValueError(f"{self.__name__}.accumulate: axis out of range")
    ...
    for i in range(rows):
      ...
      for j in range(1, axis_size):
        ...
        acc = (
          self.__call__(acc, v)
          if isinstance(acc, (builtins.int, builtins.float, builtins.bool, builtins.complex))
          else typing.cast(ndarray, self.__call__(asarray(acc), asarray(v)))._scalar()
        )
        out_flat[i, j] = acc
    res = (
      out_flat.reshape(cont.shape).transpose(_inverse_moveaxis_perm(ax, -1, n))
      if n > 1
      else out_flat.reshape(cont.shape)
    )
    if n == 1:
      res = out_flat.reshape(cont.shape)
    if out is not None:
      out[...] = res
      return out
    return res
  ...
  def reduceat(
    self, a: object, indices: Sequence[int], axis: int = 0, dtype: object = None, out: ndarray | None = None
  ) -> ndarray:
...
class ndarray:
  ...
  @staticmethod
  def _wrap(
    native: _NativeArray, base: ndarray | None = None
  ) -> ndarray:  # Hot-path constructor: skip __init__'s arg parsing for fresh op results.
  ...
  def dtype(self) -> DType:
    return _DTC[builtins.int(self._native.dtype_code())]
  ...
  @property
  def ndim(self) -> int:
    return builtins.int(self._native.ndim())

  @property
  def size(self) -> int:
    return builtins.int(self._native.size())
  ...
  @property
  def nbytes(self) -> int:
    return self.dtype.storage_nbytes(self.size)
  ...
  def __dlpack__(
    ...
    *,
    ...
    if copy is True:
      return _dlpack.export_array(asarray(self, copy=True), max_version, copied=True)
  ...
  def __getitem__(self, k: typing.Any) -> object:
    ...
    if _is_advanced_index(k):
      return _advanced_getitem(self, k)
  ...
  def __setitem__(self, k: object, v: object) -> None:
    if _is_advanced_index(k):
      return _advanced_setitem(self, k, v)
    if isinstance(k, tuple) and builtins.any(_is_advanced_index(p) for p in k):
  ...
  def __int__(self) -> int:
    if self.size != 1:
      raise TypeError(_S1E)
    s = self._scalar()
    if isinstance(s, builtins.complex):
      return builtins.int(s.real)
  ...
  def reshape(self, *shape: int | Sequence[int]) -> ndarray:
    return ndarray(_native.reshape(self._native, _shape_args(shape)), base=self)
  ...
  def astype(self, dtype: object, *, copy: builtins.bool = True, device: object = None) -> ndarray:
    ...
    if type(dtype) is DType:
      t = dtype
      if not copy and builtins.int(self._native.dtype_code()) == t.code:
        return self
      return ndarray(_native.astype(self._native, t.code))
    t = _resolve_dtype(dtype)
    if not copy and t == self.dtype:
      return self
    return ndarray(_native.astype(self._native, t.code))

  def tolist(self) -> object:
    return _unflat([self._native.get_scalar(i) for i in range(self.size)], self.shape)
  ...
  def _view_for_key(self, key: object) -> ndarray:
    ...
    if builtins.any(p is None for p in parts):
      ...
      if only_full_slices and axis == len(shape):
        out = self
        for ax in new_axes:
          out = ndarray._wrap(_native.expand_dims(out._native, ax), base=out)
        return out
    starts: list[int] = []
    stops: list[int] = []
    steps: list[int] = []
...
class _DeferredArray:
  ...
  def __setitem__(self, k: object, v: object) -> None:
    self._materialize()[k] = v

  def __bool__(self) -> builtins.bool:
    return builtins.bool(self._materialize())

  def __int__(self) -> int:
    return builtins.int(self._materialize())

  def __float__(self) -> float:
    return builtins.float(self._materialize())

  def __add__(self, o: object) -> object:
    return _binary(self, o, OP_ADD)

  def __radd__(self, o: object) -> object:
    return _binary(o, self, OP_ADD)

  def __sub__(self, o: object) -> object:
    return _binary(self, o, OP_SUB)

  def __rsub__(self, o: object) -> object:
    return _binary(o, self, OP_SUB)

  def __mul__(self, o: object) -> object:
    return _binary(self, o, OP_MUL)

  def __rmul__(self, o: object) -> object:
    return _binary(o, self, OP_MUL)

  def __truediv__(self, o: object) -> object:
    return _binary(self, o, OP_DIV)

  def __rtruediv__(self, o: object) -> object:
    return _binary(o, self, OP_DIV)
...
class _ScalarBinaryExpression(_DeferredArray):
  ...
  def __init__(
    self, array: ndarray | _DeferredArray, scalar: object, scalar_dtype: DType, op: int, scalar_on_left: builtins.bool
  ) -> None:
    super().__init__()
    self._array = array
    self._scalar = scalar
  ...
  @property
  def dtype(self) -> DType:
    return _BR[self._op][(self._array.dtype, self._scalar_dtype)]
  ...
  def _compute(self) -> ndarray:
    a = _mat(self._array)
    return ndarray(
      _native.binary_scalar(a._native, self._scalar, self._scalar_dtype.code, self._op, self._scalar_on_left)
    )
...
def result_type(*arrays_and_dtypes: object) -> DType:
  if not arrays_and_dtypes:
    raise TypeError("result_type() needs at least one array or dtype")
  strong: list[DType] = []
  weak: list[object] = []
  ...
  if strong:
    result = strong[0]
    for d in strong[1:]:
      result = promote_types(result, d)
    for value in weak:
      result = promote_types(result, _scalar_dtype_for_array_dtype(result, value))
    return result
  result = _isd(weak[0])
  for value in weak[1:]:
    result = promote_types(result, _isd(value))
  return result


def can_cast(from_: object, to: object, casting: str = "safe") -> builtins.bool:
  try:
    casting_code = _CASTING_CODES[casting]
  except KeyError as exc:
    raise ValueError(f"casting must be one of {tuple(_CASTING_CODES)}") from exc
...
def issubdtype(arg1: object, arg2: object) -> builtins.bool:
  ...
  if abstract is not None:
    return d in abstract
  try:
    return d == _resolve_dtype(arg2)
  except NotImplementedError:
    return False
...
def isdtype(dtype: object, kind: object) -> builtins.bool:
  ...
  if isinstance(kind, tuple):
    return builtins.any(isdtype(d, k) for k in kind)
  abstract = _abstract_dtype_set(kind, for_isdtype=True)
  if abstract is not None:
    return d in abstract
  if kind in (builtins.bool, builtins.int, builtins.float):
    raise TypeError(f"kind argument must be comprised of monpy dtypes or strings only, but is a {type(kind)!r}")
  try:
    return d == _resolve_dtype(kind)
  except NotImplementedError as exc:
    raise TypeError(
      f"kind argument must be comprised of monpy dtypes or strings only, but is a {type(kind)!r}"
    ) from exc
...
def finfo(dtype: object) -> _FInfo:
  ...
  try:
    ...
    raise ValueError(f"data type {d!r} not inexact") from exc
...
def iinfo(dtype: object) -> _IInfo:
  ...
  try:
    ...
    raise ValueError(f"Invalid integer data type {d.kind!r}.") from exc
...
def array(obj: object, dtype: object = None, *, copy: builtins.bool | None = True, device: object = None) -> ndarray:
  ...
  if dtype is complex128 and copy is True:
    try:
      return ndarray(_native.asarray_complex128_copy_from_buffer(obj))
    except Exception as exc:
      message = str(exc)
...
def asarray(obj: object, dtype: object = None, *, copy: builtins.bool | None = None, device: object = None) -> ndarray:
  ...
  if t is ndarray:  # already-ours fast path: skip numpy entirely
    ...
    if dtype is None:
      return arr.astype(arr.dtype, copy=True) if copy is True else arr
    tgt = _resolve_dtype(dtype)
    if tgt == arr.dtype and copy is not True:
      return arr
    if copy is False:
      raise ValueError(_CFE)
  ...
  if dtype is complex64 and copy is False:
    try:
      return ndarray(_native.asarray_complex64_view_from_buffer(obj), owner=obj)
    except Exception as exc:
      message = str(exc)
      if "copy" in message or "readonly" in message:
  ...
  try:
    ...
    pass
  if _has_ai(obj):
    return _ai_asarray(obj, dtype=dtype, copy=copy)
  if copy is False:
    raise ValueError(_CFE)
...
def _empty_native_from_shape(shape: tuple[int, ...], dtype_code: int) -> _NativeArray:
  if len(shape) == 1:
    return _native.empty_rank1(shape[0], dtype_code)
  if len(shape) == 2:
    return _native.empty_rank2(shape[0], shape[1], dtype_code)
...
def zeros(shape: int | Sequence[int], dtype: object = None, *, device: object = None) -> ndarray:
  return full(shape, 0, dtype=_resolve_dtype(dtype) if dtype is not None else float64, device=device)


def ones(shape: int | Sequence[int], dtype: object = None, *, device: object = None) -> ndarray:
  return full(shape, 1, dtype=_resolve_dtype(dtype) if dtype is not None else float64, device=device)
...
def empty_like(
  ...
  *,
  ...
  if shape is not None:
    if dtype is not None:
      return _empty_from_shape_obj(shape, _resolve_dtype(dtype).code)
...
def _full_like(
  prototype: object, fill_value: object, dtype: object, shape: int | Sequence[int] | None, device: object
) -> ndarray:
  _check_cpu(device)
  arr = prototype if type(prototype) is ndarray else asarray(prototype)
  if shape is not None:
  ...
  if dtype is None:
    return ndarray._wrap(_native.full_like(arr._native, fill_value, -1))
  t = _resolve_dtype(dtype)
...
def arange(
  ...
  *,
  ...
  t = (
    ...
    if dtype is not None
    else (float64 if builtins.any(isinstance(v, builtins.float) for v in (a, b, step)) else int64)
  )
  return ndarray(_native.arange(a, b, step, t.code))


def linspace(
  start: int | float, stop: int | float, num: int = 50, *, dtype: object = None, device: object = None
) -> ndarray:
  _check_cpu(device)
  t = _resolve_dtype(dtype) if dtype is not None else float64
...
def identity(n: int, dtype: object = None, *, device: object = None) -> ndarray:
  return eye(n, n, 0, dtype=dtype, device=device)
...
def atleast_2d(*arys: object) -> ndarray | tuple[ndarray, ...]:
  def _bump(a: object) -> ndarray:
    ...
    if arr.ndim == 1:
      return ndarray._wrap(_native.expand_dims(arr._native, 0), base=arr)
    return arr
...
def geomspace(
  ...
  *,
  ...
  t = _resolve_dtype(dtype) if dtype is not None else float64
  if num == 0:
    return asarray([], dtype=t)
  if num == 1:
    return asarray([float(start)], dtype=t)
  log_start = math.log(start)
  log_stop = math.log(stop)
  if endpoint:
    logs = [log_start + (log_stop - log_start) * i / (num - 1) for i in range(num)]
  else:
    logs = [log_start + (log_stop - log_start) * i / num for i in range(num)]
  vals = [math.exp(L) for L in logs]
  return asarray(vals, dtype=t)


def meshgrid(
  *xi: object, indexing: str = "xy", sparse: builtins.bool = False, copy: builtins.bool = True
) -> tuple[ndarray, ...]:
  ...
  if sparse:
    raise NotImplementedError("sparse meshgrid not implemented for rank != 2 in monpy v1")
  for a in arrs:
    if a.ndim > 1:
      raise ValueError("meshgrid input must be 1D")
...
def indices(
  dimensions: Sequence[int], dtype: object = None, sparse: builtins.bool = False
) -> ndarray | tuple[ndarray, ...]:
  if sparse:
    raise NotImplementedError("sparse indices not implemented in monpy v1")
  t = _resolve_dtype(dtype) if dtype is not None else int64
  if t != int64:
    raise NotImplementedError("indices dtype must be int64 in monpy v1")
  dims = tuple(dimensions)
  n = len(dims)
  if n == 0:
    return zeros((0,), dtype=t)
...
def ix_(*args: object) -> tuple[ndarray, ...]:
  ...
  for i, a in enumerate(arrs):
    if a.ndim != 1:
      raise ValueError("ix_ inputs must be 1D")
    bcast = [1] * n
    bcast[i] = a.shape[0]
...
def squeeze(x: object, axis: int | Sequence[int] | None = None) -> ndarray:
  arr = x if type(x) is ndarray else asarray(x)
  try:
    if axis is None:
      native = _native.squeeze_all(arr._native)
    elif isinstance(axis, builtins.int):
      native = _native.squeeze_axis(arr._native, axis)
    else:
...
def moveaxis(x: object, source: int | Sequence[int], destination: int | Sequence[int]) -> ndarray:
  ...
  if isinstance(source, builtins.int) and isinstance(destination, builtins.int):
    try:
      return ndarray._wrap(_native.moveaxis_single(arr._native, source, destination), base=arr)
    except Exception as exc:
      raise ValueError(str(exc)) from exc
  n = arr.ndim
  ...
  if len(src) != len(dst):
    raise ValueError("moveaxis: source/destination length mismatch")
  ...
  for axis_set in (src_norm, dst_norm):
    if builtins.any(a < 0 or a >= n for a in axis_set):
      raise ValueError("moveaxis: axis out of range")
    if len(set(axis_set)) != len(axis_set):
      raise ValueError("moveaxis: repeated axis")
  remaining = [i for i in range(n) if i not in set(src_norm)]
  perm = [-1] * n
  for s, d in builtins.zip(src_norm, dst_norm):
    perm[d] = s
  next_remain = 0
  for i in range(n):
    if perm[i] == -1:
...
def swapaxes(x: object, axis1: int, axis2: int) -> ndarray:
  arr = asarray(x)
  try:
    return ndarray._wrap(_native.swapaxes(arr._native, axis1, axis2), base=arr)
  except Exception as exc:
    raise ValueError(str(exc)) from exc
...
def ravel(x: object, order: str = "C") -> ndarray:
  if order not in ("C", "K", "A"):
    raise NotImplementedError("ravel order != C/K/A")
  arr = asarray(x)
...
def flatten(x: object, order: str = "C") -> ndarray:
  if order not in ("C", "K", "A"):
    raise NotImplementedError("flatten order != C/K/A")
  arr = asarray(x)
...
def _concat_into_flat(arrs: Sequence[ndarray], axis: int) -> tuple[list[object], tuple[int, ...]]:
  ...
  ax = axis + n if axis < 0 else axis
  if ax < 0 or ax >= n:
    raise ValueError("concatenate: axis out of range")
  for a in arrs:
    if a.ndim != n:
      raise ValueError("concatenate: arrays must have same number of dimensions")
    for d in range(n):
      if d == ax:
        continue
      if a.shape[d] != ref.shape[d]:
        raise ValueError(f"concatenate: shape mismatch on axis {d}")
...
def _is_concatenate_validation_error(message: str) -> builtins.bool:
  return message.startswith((
    "concatenate: axis out of range",
    "concatenate: arrays must have same ndim",
    "concatenate: shape mismatch",
  ))


def concatenate(
  arrays: Sequence[object], axis: int = 0, *, out: object = None, dtype: object = None, casting: str = "same_kind"
) -> ndarray:
  if out is not None:
    raise NotImplementedError("concatenate out= not implemented in monpy v1")
  ...
  if dtype is None and type(seq[0]) is ndarray:
    ...
    try:
      natives = [a._native for a in exact_arrs]
    except AttributeError:
      arrs = [asarray(a) for a in seq]
    else:
      arrs = exact_arrs
      try:
        return ndarray._wrap(_native.concatenate(natives, axis, -1))
      except Exception as exc:
        message = str(exc)
        ...
        if "dtype mismatch" in message or "c-contiguous" in message:
          ...
          if _is_concatenate_validation_error(message):
            raise ValueError(message) from exc
          raise
  else:
    arrs = [asarray(a) for a in seq]
...
def stack(arrays: Sequence[object], axis: int = 0, *, out: object = None, dtype: object = None) -> ndarray:
  if out is not None:
    raise NotImplementedError("stack out= not implemented in monpy v1")
  arrs = [asarray(a) for a in arrays]
  if not arrs:
    raise ValueError("stack: need at least one array")
  if dtype is None:
    ax0 = axis + arrs[0].ndim + 1 if axis < 0 else axis
    if ax0 == 0:
      try:
        return ndarray._wrap(
          _native.stack_axis0([a._native for a in arrs], builtins.int(arrs[0]._native.dtype_code()), False)
        )
      except Exception:
        pass
  ref = arrs[0]
  ...
  for a in arrs:
    if a.shape != ref_shape:
      raise ValueError("stack: arrays must have identical shape")
  n = ref.ndim
  ax = axis + n + 1 if axis < 0 else axis
  if ax < 0 or ax > n:
    raise ValueError("stack: axis out of range")
  if ax == 0 and n > 0:
...
def hstack(arrays: Sequence[object]) -> ndarray:
  arrs = [asarray(a) for a in arrays]
  if not arrs:
    raise ValueError("hstack: empty input")
  if arrs[0].ndim <= 1:
...
def vstack(arrays: Sequence[object]) -> ndarray:
  ...
  if raw:
    try:
      return ndarray._wrap(
        _native.stack_axis0([a._native for a in raw], builtins.int(raw[0]._native.dtype_code()), True)
      )
    except Exception:
      pass
...
def _split_indices(
  arr_size: int, indices_or_sections: int | Sequence[int], *, allow_uneven: builtins.bool
) -> list[tuple[int, int]]:
  if isinstance(indices_or_sections, builtins.int):
    n = indices_or_sections
    if n <= 0:
      raise ValueError("split: number of sections must be > 0")
    if not allow_uneven and arr_size % n != 0:
      raise ValueError("split: array does not divide evenly")
...
def array_split(ary: object, indices_or_sections: int | Sequence[int], axis: int = 0) -> list[ndarray]:
  ...
  ax = axis + n if axis < 0 else axis
  if ax < 0 or ax >= n:
    raise ValueError("array_split: axis out of range")
...
def vsplit(ary: object, indices_or_sections: int | Sequence[int]) -> list[ndarray]:
  arr = asarray(ary)
  if arr.ndim < 2:
    raise ValueError("vsplit: array must have at least 2 dimensions")
  return split(arr, indices_or_sections, axis=0)
...
def dsplit(ary: object, indices_or_sections: int | Sequence[int]) -> list[ndarray]:
  arr = asarray(ary)
  if arr.ndim < 3:
    raise ValueError("dsplit: array must have at least 3 dimensions")
...
def flip(m: object, axis: int | Sequence[int] | None = None) -> ndarray:
  arr = asarray(m)
  if axis is None:
    axes: tuple[int, ...] = ()  # () means flip all axes (mojo flip_ops convention)
  elif isinstance(axis, builtins.int):
    return ndarray._wrap(_native.flip_axis_single(arr._native, axis), base=arr)
  else:
    axes = tuple(axis)
...
def fliplr(m: object) -> ndarray:
  arr = asarray(m)
  if arr.ndim < 2:
    raise ValueError("fliplr() requires array with at least 2 dimensions")
...
def flipud(m: object) -> ndarray:
  arr = asarray(m)
  if arr.ndim < 1:
    raise ValueError("flipud() requires array with at least 1 dimension")
...
def rot90(m: object, k: int = 1, axes: tuple[int, int] = (0, 1)) -> ndarray:
  arr = asarray(m)
  if arr.ndim < 2:
    raise ValueError("rot90() requires array with at least 2 dimensions")
  ax0, ax1 = axes
  if ax0 < 0:
    ax0 += arr.ndim
  if ax1 < 0:
    ax1 += arr.ndim
  if ax0 == ax1 or ax0 < 0 or ax1 < 0 or ax0 >= arr.ndim or ax1 >= arr.ndim:
    raise ValueError("rot90() axes must be different and in bounds")
  k = k % 4
  if k == 0:
    return arr
  if k == 2:
    return flip(flip(arr, axis=ax0), axis=ax1)
  perm = list(range(arr.ndim))
  perm[ax0], perm[ax1] = perm[ax1], perm[ax0]
  if k == 1:
    return transpose(flip(arr, axis=ax1), tuple(perm))
...
def expand_dims(a: object, axis: int | Sequence[int]) -> ndarray:
  ...
  out = arr
  for ax in sorted(norm):
    out = ndarray(_native.expand_dims(out._native, ax), base=out)
  return out
...
def angle(z: object, deg: builtins.bool = False) -> ndarray:
  ...
  if a.dtype not in (complex64, complex128):
    ...
    if deg:
      flat = [math.degrees(v) for v in flat]
    return ndarray(_native.from_flat(flat, a.shape, rdt.code))
  rdt = float32 if a.dtype == complex64 else float64
  flat = []
...
def sum(
  x: object, axis: object = None, *, dtype: object = None, out: ndarray | None = None, keepdims: builtins.bool = False
) -> object:
  return _reduce_dispatch(x, axis, REDUCE_SUM, dtype=dtype, out=out, keepdims=keepdims)


def mean(
  x: object, axis: object = None, *, dtype: object = None, out: ndarray | None = None, keepdims: builtins.bool = False
) -> object:
  return _reduce_dispatch(x, axis, REDUCE_MEAN, dtype=dtype, out=out, keepdims=keepdims)


def min(x: object, axis: object = None, *, out: ndarray | None = None, keepdims: builtins.bool = False) -> object:
  return _reduce_dispatch(x, axis, REDUCE_MIN, dtype=None, out=out, keepdims=keepdims)


def max(x: object, axis: object = None, *, out: ndarray | None = None, keepdims: builtins.bool = False) -> object:
  return _reduce_dispatch(x, axis, REDUCE_MAX, dtype=None, out=out, keepdims=keepdims)


def prod(
  x: object, axis: object = None, *, dtype: object = None, out: ndarray | None = None, keepdims: builtins.bool = False
) -> object:
  return _reduce_dispatch(x, axis, REDUCE_PROD, dtype=dtype, out=out, keepdims=keepdims)


def all(x: object, axis: object = None, *, out: ndarray | None = None, keepdims: builtins.bool = False) -> object:
  return _reduce_dispatch(x, axis, REDUCE_ALL, dtype=None, out=out, keepdims=keepdims)


def any(x: object, axis: object = None, *, out: ndarray | None = None, keepdims: builtins.bool = False) -> object:
  return _reduce_dispatch(x, axis, REDUCE_ANY, dtype=None, out=out, keepdims=keepdims)


def argmax(x: object, axis: object = None, *, keepdims: builtins.bool = False) -> object:
  return _reduce_dispatch(x, axis, REDUCE_ARGMAX, dtype=None, out=None, keepdims=keepdims)


def argmin(x: object, axis: object = None, *, keepdims: builtins.bool = False) -> object:
  return _reduce_dispatch(x, axis, REDUCE_ARGMIN, dtype=None, out=None, keepdims=keepdims)
...
def std(
  ...
  *,
  ...
  if div == 0:
    res = multiply(s, float("nan"))
  else:
    res = sqrt(divide(s, div))
  if out is not None:
    out[...] = res
    return out
  return res
...
def average(
  ...
  *,
  ...
  den = (
    sum(w, axis=axis, keepdims=keepdims)
    if w.shape != arr.shape and not keepdims
    else sum(broadcast_to(w, arr.shape), axis=axis, keepdims=keepdims)
  )
  res = divide(num, den)
  if returned:
    return res, den
...
def median(x: object, axis: object = None, *, keepdims: builtins.bool = False) -> object:
  return quantile(x, 0.5, axis=axis, keepdims=keepdims)


def quantile(
  x: object, q: object, axis: object = None, *, keepdims: builtins.bool = False, method: str = "linear"
) -> object:
  if method != "linear":
    raise NotImplementedError("monpy v1 only supports method='linear'")
  arr = _mat(_av(x))
  if axis is None:
    ...
    return (
      _quantile_from_sorted(vals, q, keepdims_axes=(), orig_shape=arr.shape)
      if keepdims
      else _quantile_from_sorted(vals, q, keepdims_axes=None, orig_shape=arr.shape)
    )
...
def _quantile_from_sorted(
  sorted_vals: Sequence[float], q: object, *, keepdims_axes: object, orig_shape: tuple[int, ...]
) -> object:
  qf = _py_float(q)
  n = len(sorted_vals)
  if n == 0:
...
def percentile(
  ...
  if isinstance(q, (list, tuple)):
    qf = [_py_float(v) / 100.0 for v in q]
  else:
    qf = _py_float(q) / 100.0
  return quantile(x, qf, axis=axis, keepdims=keepdims, method=method)


def cumsum(x: object, axis: object = None, *, dtype: object = None, out: ndarray | None = None) -> object:
  return _cumulative_dispatch(x, axis, "sum", dtype=dtype, out=out)


def cumprod(x: object, axis: object = None, *, dtype: object = None, out: ndarray | None = None) -> object:
  return _cumulative_dispatch(x, axis, "prod", dtype=dtype, out=out)
...
def cummax(x: object, axis: object = None, *, out: ndarray | None = None) -> object:
  return _cumulative_minmax(x, axis, "max", out=out)


def cummin(x: object, axis: object = None, *, out: ndarray | None = None) -> object:
  return _cumulative_minmax(x, axis, "min", out=out)
...
def nansum(
  ...
  if mask is None:
    return sum(arr, axis=axis, dtype=dtype, out=out, keepdims=keepdims)
  zero = zeros_like(arr)
...
def nanmean(
  ...
  if arr.dtype not in (float32, float64):
    return mean(arr, axis=axis, dtype=dtype, out=out, keepdims=keepdims)
  mask = isnan(arr)
  zero = zeros_like(arr)
  clean = where(mask, zero, arr)
...
def nanmin(x: object, axis: object = None, *, out: ndarray | None = None, keepdims: builtins.bool = False) -> object:
  arr = asarray(x)
  if arr.dtype not in (float32, float64):
    return min(arr, axis=axis, out=out, keepdims=keepdims)
  mask = isnan(arr)
...
def nanmax(x: object, axis: object = None, *, out: ndarray | None = None, keepdims: builtins.bool = False) -> object:
  arr = asarray(x)
  if arr.dtype not in (float32, float64):
    return max(arr, axis=axis, out=out, keepdims=keepdims)
  mask = isnan(arr)
...
def nanprod(
  ...
  arr = asarray(x)
  if arr.dtype not in (float32, float64):
    return prod(arr, axis=axis, dtype=dtype, out=out, keepdims=keepdims)
  mask = isnan(arr)
...
def nanstd(
  ...
  *,
  ...
  arr = asarray(x)
  if arr.dtype not in (float32, float64):
    return std(arr, axis=axis, ddof=ddof, dtype=dtype, out=out, keepdims=keepdims)
...
def nanvar(
  ...
  *,
  ...
  arr = asarray(x)
  if arr.dtype not in (float32, float64):
    return var(arr, axis=axis, ddof=ddof, dtype=dtype, out=out, keepdims=keepdims)
...
def nanargmax(x: object, axis: object = None, *, keepdims: builtins.bool = False) -> object:
  arr = asarray(x)
  if arr.dtype not in (float32, float64):
    return argmax(arr, axis=axis, keepdims=keepdims)
  mask = isnan(arr)
  small = full_like(arr, float("-inf"))
  clean = where(mask, small, arr)
...
def nanargmin(x: object, axis: object = None, *, keepdims: builtins.bool = False) -> object:
  arr = asarray(x)
  if arr.dtype not in (float32, float64):
    return argmin(arr, axis=axis, keepdims=keepdims)
  mask = isnan(arr)
  big = full_like(arr, float("inf"))
  clean = where(mask, big, arr)
...
def nanmedian(x: object, axis: object = None, *, keepdims: builtins.bool = False) -> object:
...
def nanquantile(
  ...
  arr = asarray(x)
  if arr.dtype not in (float32, float64):
    return quantile(arr, q, axis=axis, keepdims=keepdims, method=method)
  if axis is not None:
    raise NotImplementedError("nanquantile axis= not implemented in monpy v1")
...
def nancumsum(x: object, axis: object = None, *, dtype: object = None, out: ndarray | None = None) -> object:
  arr = asarray(x)
  if arr.dtype not in (float32, float64):
    return cumsum(arr, axis=axis, dtype=dtype, out=out)
  mask = isnan(arr)
  zero = zeros_like(arr)
  clean = where(mask, zero, arr)
...
def nancumprod(x: object, axis: object = None, *, dtype: object = None, out: ndarray | None = None) -> object:
  arr = asarray(x)
  if arr.dtype not in (float32, float64):
    return cumprod(arr, axis=axis, dtype=dtype, out=out)
  mask = isnan(arr)
...
def sort(a: object, axis: int = -1, kind: str | None = None, order: object = None) -> ndarray:
  if order is not None:
    raise NotImplementedError("sort order= not implemented")
...
def argsort(a: object, axis: int = -1, kind: str | None = None, order: object = None) -> ndarray:
  if order is not None:
    raise NotImplementedError("argsort order= not implemented")
  del kind
  arr = _mat(_av(a))
  n = arr.ndim
  if n == 0:
    return ndarray(_native.from_flat([0], (), int64.code))
  if axis is None:
    flat = _flat_values(ravel(arr))
...
def partition(
  a: object, kth: int | Sequence[int], axis: int = -1, kind: str = "introselect", order: object = None
) -> ndarray:
  ...
  del kind, kth
  if order is not None:
    raise NotImplementedError("partition order= not implemented")
...
def argpartition(
  ...
  del kind, kth
  if order is not None:
    raise NotImplementedError("argpartition order= not implemented")
...
def lexsort(keys: Sequence[object], axis: int = -1) -> ndarray:
  ...
  for a in arrs:
    if a.shape != ref.shape:
      raise ValueError("lexsort: keys must share shape")
  if ref.ndim == 0:
    return ndarray(_native.from_flat([0], (), int64.code))
  if ref.ndim == 1:
    n = ref.shape[0]
...
def searchsorted(a: object, v: object, side: str = "left", sorter: object = None) -> object:
  if sorter is not None:
    raise NotImplementedError("searchsorted sorter= not implemented")
  if side not in ("left", "right"):
    raise ValueError("side must be 'left' or 'right'")
  arr = asarray(a)
  if arr.ndim != 1:
    raise ValueError("searchsorted: a must be 1D")
  ...
  for x in needles:
    if side == "left":
      out.append(bisect.bisect_left(haystack, x))
    else:
      out.append(bisect.bisect_right(haystack, x))
  if is_scalar:
    return ndarray(_native.from_flat(out, (), int64.code))._scalar()
...
def digitize(x: object, bins: object, right: builtins.bool = False) -> ndarray:
  ...
  for v in vals:
    if right:
      idx = bisect.bisect_left(bins_list, v)
    else:
      idx = bisect.bisect_right(bins_list, v)
...
def unique(
  ar: object,
  return_index: builtins.bool = False,
  return_inverse: builtins.bool = False,
  return_counts: builtins.bool = False,
  axis: object = None,
) -> object:
  if axis is not None:
    raise NotImplementedError("unique axis= not implemented")
  arr = asarray(ar)
  flat = _flat_values(ravel(arr))
  ...
  out = [u_arr]
  if return_index:
    out.append(ndarray(_native.from_flat(unique_indices, (len(unique_indices),), int64.code)))
  if return_inverse:
    out.append(ndarray(_native.from_flat(inverse, (len(inverse),), int64.code)))
  if return_counts:
    out.append(ndarray(_native.from_flat(counts, (len(counts),), int64.code)))
  return tuple(out)
...
def bincount(x: object, weights: object = None, minlength: int = 0) -> ndarray:
  ...
  if w.shape != arr.shape:
    raise ValueError("bincount: weights shape mismatch")
  out_f = [0.0] * n
  for i, v in enumerate(vals):
    out_f[v] += builtins.float(w._native.get_scalar(i))
  return ndarray(_native.from_flat(out_f, (n,), float64.code))


def isin(
  element: object, test_elements: object, assume_unique: builtins.bool = False, invert: builtins.bool = False
) -> ndarray:
  arr = asarray(element)
  ...
  vals = _flat_values(ravel(arr))
  if invert:
    res = [v not in test_set for v in vals]
  else:
    res = [v in test_set for v in vals]
  return ndarray(_native.from_flat(res, arr.shape, bool.code))


def intersect1d(
  ar1: object, ar2: object, assume_unique: builtins.bool = False, return_indices: builtins.bool = False
) -> object:
  ...
  return (
    arr,
    ndarray(_native.from_flat(i1, (len(i1),), int64.code)),
    ndarray(_native.from_flat(i2, (len(i2),), int64.code)),
  )
...
def nonzero(a: object) -> tuple[ndarray, ...]:
  ...
  n = arr.ndim
  if n == 0:
    if builtins.bool(flat[0]):
      return tuple(ndarray(_native.from_flat([0], (1,), int64.code)) for _ in range(builtins.max(1, n)))
...
def argwhere(a: object) -> ndarray:
  ...
  parts = nonzero(arr)
  if not parts:
    return ndarray(_native.from_flat([], (0, arr.ndim), int64.code))
  n = arr.ndim
  count = parts[0].size
  flat: list[int] = []
...
def ravel_multi_index(
  multi_index: Sequence[object], dims: Sequence[int], mode: str = "raise", order: str = "C"
) -> ndarray:
  if order != "C":
    raise NotImplementedError("ravel_multi_index order != 'C'")
  if mode != "raise":
    raise NotImplementedError("ravel_multi_index mode != 'raise'")
  axes = [asarray(a) for a in multi_index]
  if not axes:
    raise ValueError("multi_index is empty")
  shape = axes[0].shape
  n = len(dims)
  if n != len(axes):
    raise ValueError("multi_index / dims length mismatch")
  ...
  for i in range(cnt):
    ...
    for d in range(n):
      ...
      if v < 0 or v >= dims[d]:
        raise ValueError("ravel_multi_index: index out of bounds")
      idx += v * strides[d]
    out.append(idx)
  return ndarray(_native.from_flat(out, shape, int64.code))
...
def unravel_index(indices: object, shape: Sequence[int], order: str = "C") -> tuple[ndarray, ...]:
  if order != "C":
    raise NotImplementedError("unravel_index order != 'C'")
  arr = asarray(indices)
  flat = _flat_values(ravel(arr))
...
def _normalize_axis_index(value: object, axis_size: int, axis: int) -> int:
  ...
  if idx < 0 or idx >= axis_size:
    raise IndexError(f"index {raw} is out of bounds for axis {axis} with size {axis_size}")
  return idx


def _broadcast_coord(
  coords: tuple[int, ...], source_shape: tuple[int, ...], target_shape: tuple[int, ...], context: str
) -> tuple[int, ...]:
  if not source_shape:
    return ()
  if len(source_shape) > len(target_shape):
    raise ValueError(f"{context}: value array could not be broadcast to indexing result")
...
def take(
  a: object, indices: object, axis: object = None, *, out: ndarray | None = None, mode: str = "raise"
) -> ndarray:
  if mode != "raise":
    raise NotImplementedError("take mode != 'raise'")
  arr = asarray(a)
  idx = asarray(indices)
  if axis is None:
    flat = _flat_values(ravel(arr))
    ...
    for r in range(rows):
      row = [flat._native.get_scalar(r * axis_size + i) for i in range(axis_size)]
      for i in idx_flat:
        out_vals.append(row[builtins.int(i)])
...
def take_along_axis(arr: object, indices: object, axis: object) -> ndarray:
  ...
  n = a.ndim
  if idx.ndim != n:
    raise ValueError("take_along_axis: indices.ndim must match arr.ndim")
  ax = _axis_int(axis, n, "take_along_axis")
  out_shape = [0] * n
  for d in range(n):
    if d == ax:
      out_shape[d] = idx.shape[d]
    elif a.shape[d] == idx.shape[d] or idx.shape[d] == 1:
      out_shape[d] = a.shape[d]
    elif a.shape[d] == 1:
      out_shape[d] = idx.shape[d]
    else:
      raise ValueError("take_along_axis: shape mismatch on non-axis dim")
...
def put_along_axis(arr: ndarray, indices: object, values: object, axis: object) -> None:
  ...
  if not isinstance(arr, ndarray):
    raise TypeError("put_along_axis: arr must be ndarray")
  idx = asarray(indices)
  vals = asarray(values)
  if axis is None:
    ...
    if not flat_v:
      raise ValueError("put_along_axis: empty values")
    if len(flat_v) < len(flat_i):
      flat_v = (flat_v * ((len(flat_i) + len(flat_v) - 1) // len(flat_v)))[: len(flat_i)]
  ...
  n = arr.ndim
  if idx.ndim != n:
    raise ValueError("put_along_axis: indices.ndim must match arr.ndim")
  ax = _axis_int(axis, n, "put_along_axis")
  iter_shape = [0] * n
  for d in range(n):
    if d == ax:
      iter_shape[d] = idx.shape[d]
    elif arr.shape[d] == idx.shape[d] or idx.shape[d] == 1:
      iter_shape[d] = arr.shape[d]
    elif arr.shape[d] == 1:
      iter_shape[d] = idx.shape[d]
    else:
      raise IndexError("shape mismatch: indexing arrays could not be broadcast")
...
def put(arr: ndarray, indices: object, values: object, mode: str = "raise") -> None:
  ...
  if mode != "raise":
    raise NotImplementedError("put mode != 'raise'")
  if not isinstance(arr, ndarray):
    raise TypeError("put: arr must be ndarray")
  flat_i = _flat_values(ravel(asarray(indices)))
  flat_v = _flat_values(ravel(asarray(values)))
  n = len(flat_i)
  if not flat_v:
    raise ValueError("put: cannot use empty values array")
  if len(flat_v) < n:
    flat_v = (flat_v * ((n + len(flat_v) - 1) // len(flat_v)))[:n]
  for i, v in builtins.zip(flat_i, flat_v):
    mi = _flat_to_multi(_normalize_axis_index(i, arr.size, 0), arr.shape)
    arr[mi] = v
...
def block(arrays: object) -> ndarray:
  ...
  d = _md(arrays)
  if d == 0:
    return atleast_1d(asarray(arrays))
...
def repeat(a: object, repeats: object, axis: object = None) -> ndarray:
  ...
  flat = moved.reshape((rows, axis_size))
  if isinstance(repeats, builtins.int):
    reps = [repeats] * axis_size
  else:
    reps = [builtins.int(v) for v in _flat_values(asarray(repeats))]
  if len(reps) != axis_size:
    raise ValueError("repeat: repeats length must match axis")
...
def tile(A: object, reps: int | Sequence[int]) -> ndarray:
  ...
  while arr.ndim < len(reps_t):
    arr = expand_dims(arr, 0)
  while len(reps_t) < arr.ndim:
    reps_t = (1,) + reps_t
  ...
  for d in range(arr.ndim - 2, -1, -1):
    in_strides[d] = in_strides[d + 1] * arr.shape[d + 1]
  out = []
  for i in range(total):
    rem = i
    src_idx = 0
...
def roll(a: object, shift: int | Sequence[int], axis: object = None) -> ndarray:
  arr = asarray(a)
  if axis is None:
    if not isinstance(shift, builtins.int):
      raise ValueError("roll: shift must be int when axis=None")
    n = arr.size
    if n == 0:
      return arr
    s = shift % n
  ...
  if len(axes) != len(shifts):
    raise ValueError("roll: shift/axis length mismatch")
  out = arr
...
def append(arr: object, values: object, axis: object = None) -> ndarray:
  if axis is None:
    return concatenate([ravel(asarray(arr)), ravel(asarray(values))])
  base = asarray(arr)
...
def insert(arr: object, obj: object, values: object, axis: object = None) -> ndarray:
  ...
  if axis is None:
    ...
    ax = _axis_int(axis, base.ndim, "insert")
  if isinstance(obj, builtins.int):
    positions = [obj]
  else:
    positions = [_py_int(p) for p in _iter(obj)]
  vals = asarray(values)
  if vals.ndim == 0:
    vals = expand_dims(vals, 0)
  pieces = []
  prev = 0
  flat_pos = sorted(positions)
  if vals.shape[0] == 1 and len(flat_pos) > 1:
    vals = tile(vals, len(flat_pos))
  for i, p in enumerate(flat_pos):
    ...
    if vals.ndim == base.ndim:
      pieces.append(vals[val_slice_key])
    else:
      bcast_shape = list(base.shape)
      bcast_shape[ax] = 1
      pieces.append(
        broadcast_to(
          vals.reshape((1,) * (base.ndim - vals.ndim) + vals.shape) if vals.ndim < base.ndim else vals[val_slice_key],
          tuple(bcast_shape),
        )
      )
    prev = p
...
def delete(arr: object, obj: object, axis: object = None) -> ndarray:
  ...
  if axis is None:
    ...
    ax = _axis_int(axis, base.ndim, "delete")
  if isinstance(obj, builtins.int):
    to_drop = {obj if obj >= 0 else obj + base.shape[ax]}
  elif isinstance(obj, slice):
    to_drop = set(range(*obj.indices(base.shape[ax])))
  else:
    to_drop = set()
    for p in _iter(obj):
      pp = _py_int(p)
...
def trim_zeros(filt: object, trim: str = "fb") -> ndarray:
  arr = asarray(filt)
  if arr.ndim != 1:
    raise ValueError("trim_zeros: input must be 1D")
  flat = _flat_values(arr)
  n = len(flat)
  start, end = 0, n
  if "f" in trim.lower():
    while start < end and not builtins.bool(flat[start]):
      start += 1
  if "b" in trim.lower():
    while end > start and not builtins.bool(flat[end - 1]):
      end -= 1
  out = flat[start:end]
  return ndarray(_native.from_flat(out, (len(out),), arr.dtype.code))
...
def broadcast_arrays(*args: object) -> list[ndarray]:
  ...
  shape = arrs[0].shape
  for a in arrs[1:]:
    shape = _broadcast_pair_shape(shape, a.shape)
...
def _broadcast_pair_shape(s1: tuple[int, ...], s2: tuple[int, ...]) -> tuple[int, ...]:
  ...
  for a, b in builtins.zip(p1, p2):
    if a == b:
      ...
      raise ValueError(f"shapes {s1} and {s2} cannot be broadcast")
  return tuple(out)


def _pad_axis_indices(n: int, pre: int, post: int, mode: str) -> list[int]:
  if pre < 0 or post < 0:
    raise ValueError("pad: index can't contain negative values")
  if pre == 0 and post == 0:
    return list(range(n))
  if n <= 0:
    raise ValueError(f"{mode}: can't extend empty axis")
  if mode == "edge":
    return [0] * pre + list(range(n)) + [n - 1] * post
  if mode == "wrap":
    return [p % n for p in range(-pre, n + post)]
  if mode == "symmetric":
    period = 2 * n
    return [(m if m < n else period - 1 - m) for m in (p % period for p in range(-pre, n + post))]
  if mode == "reflect":
    if n <= 1:
      raise ValueError("reflect: axis size must be >= 2")
    period = 2 * n - 2
...
def pad(array: object, pad_width: object, mode: str = "constant", **kwargs: object) -> ndarray:
  ...
  if isinstance(pad_width, builtins.int):
    pw = [(pad_width, pad_width)] * arr.ndim
  else:
    items = list(_iter(pad_width))
    if items and not isinstance(items[0], (list, tuple)):
      pw = [(_py_int(items[0]), _py_int(items[1]))] * arr.ndim
    else:
      pw = [
        (_py_int(typing.cast(Sequence[object], p)[0]), _py_int(typing.cast(Sequence[object], p)[1])) for p in items
      ]
  if len(pw) != arr.ndim:
    raise ValueError("pad: pad_width must match ndim")
...
def tril(m: object, k: int = 0) -> ndarray:
  ...
  if arr.ndim < 2:
    raise ValueError("tril: input must be 2D")
...
def triu(m: object, k: int = 0) -> ndarray:
  arr = ascontiguousarray(asarray(m))
  if arr.ndim < 2:
    raise ValueError("triu: input must be 2D")
...
def tril_indices(n: int, k: int = 0, m: int | None = None) -> tuple[ndarray, ndarray]:
  ...
  for i in range(n):
    for j in range(builtins.min(M, i + k + 1)):
      rows.append(i)
      cols.append(j)
  return ndarray(_native.from_flat(rows, (len(rows),), int64.code)), ndarray(
    _native.from_flat(cols, (len(cols),), int64.code)
  )
...
def frombuffer(buffer: object, dtype: object = float64, count: int = -1, offset: int = 0) -> ndarray:
  t = _resolve_dtype(dtype)
  try:
    return ndarray(_native.frombuffer(buffer, t.code, int(count), int(offset)), owner=buffer)
  except Exception as exc:
    raise ValueError(str(exc)) from exc
...
def _is_advanced_index(k: object) -> builtins.bool:
  if isinstance(k, ndarray) and (k.dtype == bool or k.dtype in (int8, int16, int32, int64)):
    return True
  if isinstance(k, list) and k and builtins.all(isinstance(v, builtins.int) for v in k):
    return True
  return False
...
def _advanced_getitem(self: ndarray, k: object) -> ndarray:
  if isinstance(k, ndarray) and k.dtype == bool:
    if k.shape != self.shape[: k.ndim]:
      raise IndexError("boolean index shape mismatch")
    ...
    for i in indices:
      ...
      for j in range(rest_size):
        out_vals.append(flat_arr[base + j])
...
def _advanced_getitem_tuple(self: ndarray, k: tuple) -> ndarray:
  ...
  while len(parts) < self.ndim:
    parts.append(slice(None))
  if builtins.any(p is None for p in parts):
    raise NotImplementedError("newaxis with fancy indexing not supported in v1")
  ...
  idx_arrays = []
  for p in parts:
    if isinstance(p, builtins.int):
      idx_arrays.append(asarray([p], dtype=int64))
    elif isinstance(p, (list, ndarray)):
      idx_arrays.append(_to_int_indices(p))
    elif isinstance(p, slice):
      a, b, c = p.indices(self.shape[len(idx_arrays)])
      idx_arrays.append(asarray(list(range(a, b, c)), dtype=int64))
    else:
      raise NotImplementedError(f"unsupported index part: {p!r}")
  ...
  for i in range(total):
    ...
    for d in range(self.ndim):
      phys += builtins.int(flat_idxs[d][i]) * strides[d]
...
def _advanced_setitem(self: ndarray, k: object, v: object) -> None:
  ...
  for ii in idx_flat:
    ...
    for j in range(rest_size):
      ...
      rem = j
      rs = [1] * len(rest)
      for d in range(len(rest) - 2, -1, -1):
        rs[d] = rs[d + 1] * rest[d + 1]
...
def matmul(x1: object, x2: object) -> object:
  ...
  if type(x1) is ndarray and type(x2) is ndarray:
    return _matmul_result(x1, x2)  # ndarray x ndarray fast path; mojo handles promotion
  l = _mat(_av(x1))
  r = _mat(_av(x2))
...
def diagonal(a: object, offset: int = 0, axis1: int = 0, axis2: int = 1) -> ndarray:
  ...
  return (
    ndarray(d(arr._native, int(offset), int(axis1), int(axis2)))
    if d is not None
    else _diag_fallback(arr, int(offset), int(axis1), int(axis2))
  )


def trace(
  a: object, offset: int = 0, axis1: int = 0, axis2: int = 1, dtype: object = None, out: ndarray | None = None
) -> object:
  arr = asarray(a)
...
def ascontiguousarray(a: object, dtype: object = None, *, device: object = None) -> ndarray:
  ...
  if arr._native.is_c_contiguous():
    return arr
  return arr.astype(arr.dtype, copy=True)
...
def __array_namespace_info__() -> object:
  ...
  return SimpleNamespace(
    ...
    dtypes=dts,
    default_dtypes=ddts,
    capabilities=lambda: {"boolean indexing": False, "data-dependent shapes": False},
  )
...
def _binary_from_array(
  arr: ndarray | _DeferredArray, other: object, op: int, *, scalar_on_left: builtins.bool
) -> object:
  ...
  if _is_scalar(other):
    ...
    if op == OP_MUL and _can_def_sb(arr, sd):
      return _ScalarBinaryExpression(
        arr, other, sd, op, scalar_on_left
      )  # defer (only mul, only floats) so a downstream sin(...)+(scalar*y) can fuse
    av = _mat(arr)
...
def _unary(x: object, op: int) -> object:
  a = _av(x)
  if op == UNARY_SIN and a.dtype in (float32, float64):
    return _UnaryExpression(a, op)  # only defer sin on floats — that's what sin_add_mul fuses
  av = _mat(a)
...
def _isarrv(v: object) -> typing.TypeGuard[ndarray | _DeferredArray]:
  return isinstance(v, (ndarray, _DeferredArray))  # is "array value": ours, materialised or not


def _av(v: object) -> ndarray | _DeferredArray:
  return (
    v if isinstance(v, (ndarray, _DeferredArray)) else asarray(v)
  )  # as-array-value: convert non-arrays via asarray


def _mat(v: ndarray | _DeferredArray) -> ndarray:
  return (
    v._materialize() if isinstance(v, _DeferredArray) else v
  )  # materialize: collapse a deferred node to a concrete ndarray
...
def _coerce(l: ndarray, r: ndarray, op: int) -> tuple[ndarray, ndarray]:
  ...
  try:
    ...
    t = _DTC[_native._result_dtype_for_binary(l.dtype.code, r.dtype.code, op)]
  if l.dtype != t:
    l = l.astype(t)
  if r.dtype != t:
    r = r.astype(t)
...
def _result_dtype_for_binary(l: DType, r: DType, op: int) -> DType:
  try:
    ...
    return _DTC[_native._result_dtype_for_binary(l.code, r.code, op)]
...
def _can_def_sb(v: ndarray | _DeferredArray, sd: DType) -> builtins.bool:
  return v.dtype in (float32, float64) and sd in (
    float32,
    float64,
  )  # defer scalar binary only on float×float — int paths must materialise
...
def _diag_fallback(arr: ndarray, offset: int, axis1: int, axis2: int) -> ndarray:
  ...
  for prefix in _iter_idx(rsh):
    ...
    for a, i in zip(ra, prefix, strict=True):
      key[a] = i
    for di in range(dl):
      key[axis1] = rs + di
      key[axis2] = cs + di
...
def _trace_fallback(arr: ndarray, offset: int, axis1: int, axis2: int, dt: object) -> object:
  ...
  t = (
    _resolve_dtype(dt) if dt is not None else (int64 if d.dtype == bool else d.dtype)
  )  # bool inputs accumulate as int64 to match numpy
  if d.dtype != t:
    d = d.astype(t)
  if d.ndim == 1:
    return sum(d)
  ...
  for prefix in _iter_idx(out_shape):
    ...
    for di in range(d.shape[-1]):
      total += typing.cast(_Scalar, d[prefix + (di,)])
    flat.append(total)
  return ndarray(_native.from_flat(flat, out_shape, t.code))


def _is_scalar_value(v: object) -> builtins.bool:
  return isinstance(v, (builtins.bool, builtins.int, builtins.float, builtins.complex))


def _is_scalar(v: object) -> builtins.bool:
  return isinstance(v, (builtins.bool, builtins.int, builtins.float))  # python real scalar (bool/int/float)


def _isd(v: object) -> DType:  # infer scalar dtype from a python scalar alone
  if isinstance(v, builtins.bool):
    return bool
...
def _scalar_dtype_for_array_dtype(ad: DType, v: object) -> DType:
  ...
  if ad in (float32, float64) and isinstance(v, (builtins.bool, builtins.int, builtins.float)):
    return ad
  if ad == int64 and isinstance(v, (builtins.bool, builtins.int)):
    return int64
  return _isd(v)
...
def _dtype_for_result_type_arg(v: object) -> tuple[DType, builtins.bool]:
  if isinstance(v, (ndarray, _DeferredArray)):
    return v.dtype, True
  if _is_scalar(v):
    return _isd(v), False
  if isinstance(v, DType):
    return v, True
  if isinstance(v, str):
    return _resolve_dtype(v), True
  if _has_ai(v):
    return _dtype_from_typestr(_ai_typestr(_array_interface(v))), True
  try:
    return _resolve_dtype(v), True
  except NotImplementedError:
    pass
  try:
    return _infer_dtype(_flat(v)[1]), True
  except NotImplementedError as exc:
    raise NotImplementedError(f"unsupported result_type argument: {v!r}") from exc
...
def _dtype_for_can_cast_arg(v: object) -> DType:
  ...
  if isinstance(v, (ndarray, _DeferredArray)):
    return v.dtype
  if _has_ai(v):
    return _dtype_from_typestr(_ai_typestr(_array_interface(v)))
...
def _abstract_dtype_set(v: object, *, for_isdtype: builtins.bool) -> set[DType] | None:
  ...
  if not for_isdtype:
    if v is builtins.bool:
      return {bool}
    if v is builtins.int:
      return {int64}
    if v is builtins.float:
      return {float64}
  return None
...
def _array_interface(o: object) -> dict[str, object]:
  ...
  if not isinstance(i, dict):
    raise NotImplementedError("object __array_interface__ must be a dict")
  return i


def _ai_shape(iface: dict[str, object]) -> tuple[int, ...]:
  sh = iface.get("shape")
  if not isinstance(sh, tuple) or not builtins.all(isinstance(d, builtins.int) for d in sh):
    raise NotImplementedError("array interface shape must be a tuple of ints")
...
def _ai_strides(iface: dict[str, object]) -> tuple[int, ...] | None:
  ...
  if st is None:
    return None
  if not isinstance(st, tuple) or not builtins.all(isinstance(s, builtins.int) for s in st):
    raise NotImplementedError("array interface strides must be a tuple of ints or None")
...
def _ai_typestr(iface: dict[str, object]) -> str:
  ...
  if not isinstance(ts, str):
    raise NotImplementedError("array interface typestr must be a string")
  return ts


def _ai_data(iface: dict[str, object]) -> tuple[int, builtins.bool]:
  data = iface.get("data")
  if not isinstance(data, tuple) or len(data) != 2 or not isinstance(data[0], builtins.int):
    raise NotImplementedError("array interface data must be an address tuple")
...
def _ext_from_ai(
  a: object, d: DType, *, iface: dict[str, object] | None = None, data_address: int | None = None
) -> ndarray:
  # zero-copy borrow: native side wraps the foreign buffer; we keep `a` as owner so it isn't GC'd.
  if iface is None:
    iface = _array_interface(a)
  sh = _ai_shape(iface)
...
def _copy_from_ai(a: object, *, dtype_value: DType | None = None, iface: dict[str, object] | None = None) -> ndarray:
  ...
  if dtype_value is None:
    dtype_value = _dtype_from_typestr(_ai_typestr(iface))
  sh = _ai_shape(iface)
  rs = _ai_strides(iface)
...
def _norm_shape(shape: int | Sequence[int]) -> tuple[int, ...]:
  ...
  if builtins.any(d < 0 for d in out):
    raise ValueError("negative dimensions are not allowed")
  return out
...
def _shape_args(sh: Sequence[int | Sequence[int]]) -> tuple[int, ...]:
  ...
  if len(sh) == 1 and not isinstance(sh[0], builtins.int):
    return _norm_shape(sh[0])
...
def _flat(obj: object) -> tuple[tuple[int, ...], list[object]]:
  ...
  if isinstance(obj, (list, tuple)):
    ...
    for it in obj:
      ...
      fl.extend(f)
    fs = cs[0]
    if builtins.any(s != fs for s in cs):
...
def _norm_idx(i: object, d: int) -> int:
  if not isinstance(i, builtins.int):
...
def _check_cpu(d: object) -> None:
  if d not in (None, "cpu"):
    raise NotImplementedError("monpy v1 only supports cpu arrays")


def _check_order(order: str) -> None:
  if order not in ("C", "A", "K"):
    raise NotImplementedError("monpy v1 only supports c-contiguous materialization")


def _is_kernel_value(v: object) -> builtins.bool:
  return builtins.bool(getattr(v, "__monpy_kernel_tensor__", False))


def _has_kernel_arg(args: Sequence[object]) -> builtins.bool:

...
def _check_impl(impl: str) -> str:
  if impl != _DEFAULT_IMPL:
    raise NotImplementedError("monpy.random v1 only supports threefry2x32")
  return impl
...
def _shape(size: object) -> tuple[int, ...]:
  ...
  if isinstance(size, builtins.int):
    return (builtins.int(size),)
  if _seq(size):
    return tuple(_py_int(dim) for dim in cast(Sequence[object], size))
...
def _float_dtype(dtype: object) -> DType:
  return _dtype(dtype, _FLOAT_DTYPES, "random floating dtype must be float32 or float64")


def _bits_dtype(dtype: object) -> DType:
  return _dtype(dtype, _BITS_DTYPES, "bits dtype must be uint32 or uint64")


def _int_dtype(dtype: object) -> DType:
  return _dtype(dtype, _INT_DTYPES, "random integer dtype must be an integer dtype")
...
class KeyBatch:
  ...
  def __len__(self) -> int:
    if not self.shape:
      raise TypeError("len() of unsized KeyBatch")
    return self.shape[0]
  ...
  def __getitem__(self, index: object) -> Key | KeyBatch:
    ...
    if item.shape == (2,):
      return _key_from_words(_py_int(item[0]), _py_int(item[1]), self.impl)
...
def _key_from_data(data: ndarray, impl: str) -> Key:
  if data.dtype != uint32 or data.shape != (2,):
    raise ValueError("key data must have dtype uint32 and shape (2,)")
...
def _parse(
  name: str, args: tuple[object, ...], key: Key | None, size: object, shape_first: bool = True
) -> tuple[list[object], Key | None, object]:
  values = list(args)
  if values and isinstance(values[0], Key):
    if key is not None:
      raise TypeError(f"{name}() got multiple keys")
    key = cast(Key, values.pop(0))
  if shape_first and key is not None and values and _seq(values[0]) and size is None:
    size = values.pop(0)
...
def split(key: Key, num: int = 2) -> KeyBatch:
  ...
  if count < 0:
    raise ValueError("split num must be non-negative")
...
def wrap_key_data(data: object, *, impl: str = _DEFAULT_IMPL) -> Key | KeyBatch:
  ...
  if arr.shape == (2,):
    return _key_from_data(arr, impl)
  if arr.ndim >= 2 and arr.shape[-1] == 2:
    return KeyBatch(arr, impl)
...
def _uniform_array(source: Key, size: object, dtype: object, low: object, high: object) -> ndarray:
  ...
  return ndarray(
    _native._random_uniform(source.word0, source.word1, _shape(size), t.code, _py_float(low), _py_float(high))
  )
...
def random(key: object = None, size: object = None, dtype: object = float64) -> _FloatOrArray:
  ...
  if key is not None:
    if size is not None:
      raise TypeError("random() got both positional size and size=")
    size = key
...
def random_sample(size: object = None) -> _FloatOrArray:
  return random(None, size=size, dtype=float64)
...
def sample(size: object = None) -> _FloatOrArray:
  return random(None, size=size, dtype=float64)
...
def ranf(size: object = None) -> _FloatOrArray:
  return random(None, size=size, dtype=float64)
...
def standard_normal(size: object = None, dtype: object = float64, *, key: Key | None = None) -> _FloatOrArray:
  if isinstance(size, Key):
    if key is not None:
      raise TypeError("standard_normal() got multiple keys")
    key = size
    if _seq(dtype):
      size = dtype
      dtype = float64
      ...
      size = None
  return cast(
    _FloatOrArray, _maybe_scalar(_normal_array(key if key is not None else _next_key(), size, dtype), key, size)
  )
...
def uniform(
  *args: object,
  ...
  if values:
    size = values.pop(0)
  if values:
    raise TypeError("uniform() takes at most key, low, high, and size positional arguments")
  return cast(
    _FloatOrArray,
    _maybe_scalar(_uniform_array(key if key is not None else _next_key(), size, dtype, low, high), key, size),
  )
...
def normal(
  *args: object,
  ...
  if values:
    size = values.pop(0)
  if values:
    raise TypeError("normal() takes at most key, loc, scale, and size positional arguments")
  return cast(
    _FloatOrArray,
    _maybe_scalar(_scaled(_normal_array(key if key is not None else _next_key(), size, dtype), loc, scale), key, size),
  )
...
def randint(
  *args: object,
  ...
  if low is None:
    if not values:
      raise TypeError("randint() missing required argument 'low'")
    low = values.pop(0)
  if values:
    high = values.pop(0)
  if values:
    size = values.pop(0)
  if values:
    raise TypeError("randint() takes at most key, low, high, and size positional arguments")
  lo: object
  hi: object
  if high is None:
    lo = 0
    ...
    hi = high
  return cast(
    _IntOrArray, _maybe_scalar(_randint_array(key if key is not None else _next_key(), lo, hi, size, dtype), key, size)
  )
...
def rand(*dims: int) -> _FloatOrArray:
  return random(None, size=dims if dims else None, dtype=float64)
...
def randn(*dims: int) -> _FloatOrArray:
  return standard_normal(size=dims if dims else None, dtype=float64)
...
class Generator:
  ...
  def __init__(self, seed: object = None) -> None:
    if isinstance(seed, Key):
      ...
      self._key = key(_fresh_seed())
    else:
      self._key = key(_py_int(seed))
  ...
  @overload
  def integers(
    self, low: object, high: object | None = None, size: None = None, dtype: object = int64, endpoint: bool = False
  ) -> builtins.int: ...
  @overload
  def integers(
    self,
  ...
  def integers(
    self, low: object, high: object | None = None, size: object = None, dtype: object = int64, endpoint: bool = False
  ) -> _IntOrArray:
    if high is None:
      lo, hi = 0, low
    else:
